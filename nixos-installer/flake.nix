{
  ...
  in {
    nixosConfigurations = {
      ...
      # Swap size is in GiB
      constellation = nixpkgs.lib.nixosSystem {
        system = "x86_64-linux";
        specialArgs = minimalSpecialArgs;
        modules = [
          inputs.disko.nixosModules.disko
          ...
          ./minimal-configuration.nix
          {
            networking.hostName = "constellation";
            networking.hostId = "9a5b5e9c";
          }
          (configLib.relativeToRoot "hosts/constellation/hardware-configuration.nix")
        ];
      };
